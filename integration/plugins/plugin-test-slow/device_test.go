...
const pluginID = "plugin-test-slow"
...
func TestDeviceCreateAndMetadata(t *testing.T) {
	...
	dev := types.Device{
		ID:         deviceID,
		SourceID:   "src-slow-1",
		SourceName: "Slow Source 1",
		LocalName:  "Slow Local 1",
	}
	...
	for _, got := range devices {
		if got.ID == deviceID {
			if got.Config.Meta != "slow-metadata" {
				t.Fatalf("device metadata mismatch: got=%q want=%q", got.Config.Meta, "slow-metadata")
			}
			return
		}
	}
